<?php
	...
	$idCarPed=0;
	$noPedido= $carrito->incrementaNoPedido("expo");
				
                $sql = " insert into pedidos (idPed,idTienda,fechaPedido,sitioPedido,idVendedor,idCarPed,pares,totalPedido,seccionCatalogo,levantoPedido,lugarPedido,fechaRegistro) values ('$noPedido',$idTienda,'$fechahora','$sitioPedido',$idVendedor,$idCarPed,$TotalPares,$subtotal,'$seccionCatalogo','$txtPedidoCargo','$comboLugarPedido','$fechahora') ";

	$result_data=$mysqli->query($sql);
	
	echo  "Cliente: $cliente tienda: $tienda - Pedido :$noPedido - ";
	...
	if($resultDetalle=$mysqli->query($sqlDetallePedido)){
		...
		while($obj=$resultDetalle->fetch_object()){
			...
			$sqlGuardarDetalleNewPedido= " insert into pedidos_detalle (idEstilo,N2,N2m,N3,N3m,N4,N4m,N5,N5m,N6,N6m,pares,precio,total,idPedido) values ($idEstilo,$N2,$N2m,$N3,$N3m,$N4,$N4m,$N5,$N5m,$N6,$N6m,$pares,$precio,$total,'$noPedido')";
