	...
	/*function actulualizarColumnaFiltrada(){
		...
		for(k=0;k<arrayCamposFiltrados.length;k++){
			for(n=objJSONUltimoFiltro.length-1;n>=0;n--){
				...
				if(esDatoFiltrado(arrayCamposFiltrados[k], eval('objJSONTemp[n].'+arrayCamposFiltrados[k]))){
					if(!buscarElementos(arrayDatosFiltradosPorColumna,arrayCamposFiltrados[k])){
						arrayDatosFiltradosPorColumna.push( eval('objJSONTemp[n].'+arrayCamposFiltrados[k]));	
						var ca=i+1;
						var ca=i+1;
			...
			document.getElementById("divInVisFiltro" + arrayCamposFiltrados[k]).innerHTML = strDivFiltro;
