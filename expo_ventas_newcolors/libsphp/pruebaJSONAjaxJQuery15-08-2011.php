...
//		document.getElementById('apDiv3').innerHTML=StrArray;
	...
	function agregarCelda(row,nameColumna,j,objJSON){
		...
		var cell=row.insertCell(j+1);cell.innerHTML="<div style='position:realtive;' ></div><div style='position:relative;' onclick='cargarFiltro(objJSON ,\"" + nameColumna + "\")' > "+ nameColumna + " <img id='imgFiltro" + nameColumna + "' src='../carrito/img/icono-filtro.jpg' width='8' height='6' /> </div>" +  strDivFiltro + " ";
	...
	function agregarRowFormat(tabla,objJSONt,i){
			...
			cadena = cadena + "<td><img src='http://201.120.55.76/imagenes_system/muestras/miniminis/"+nameFotoImageGif(objJSONt[i].Estilo,objJSONt[i].Material,objJSONt[i].Color)+"' width='48' height='48'></td>"
			for(x=0;x<columnas.length;x++){
...
<body>
<div id="apDiv1" style="display:none"></div>
<div id="apDiv3" style="display:none"></div>
<div id="apDiv2"></div>
<div id="apDiv4">
  <label>
    <input type="submit" name="btnValidar" id="btnValidar" value="Ver datos" onClick="cargarJSON();"/>
    <input type="submit" name="btnQuitarFiltro" id="btnQuitarFiltro" value="Quitar ultimo filtro" onClick="quitarUltimoFiltro();">
    <input type="checkbox" name="verFoto" id="verFoto">
    Ver Foto
  </label>
  <div>
